...
+ (OTPAuthURL *)authURLWithKeychainItemRef:(NSData *)data {
  ...
  NSDictionary *result = nil;
    ...
    // FIXED FOR ARC (CFTypeRef*) to (void *)
...
- (BOOL)saveToKeychain {
  ...
  if ([self isInKeychain]) {
    ...
    _GTMDevLog(@"SecItemUpdate(%@, %@) = %d", query, attributes, (int)status);
  } else {
    ...
    _GTMDevLog(@"SecItemAdd(%@, %@) = %d", attributes, ref, (int)status);
...
- (BOOL)removeFromKeychain {
  ...
  _GTMDevLog(@"SecItemDelete(%@) = %d", query, (int)status);
